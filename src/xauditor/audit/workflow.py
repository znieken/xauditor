...
class AuditWorkflow:
    def __init__(
        ...
        *,
        ...
        # ``allow-zero-exploiter-replication``: the exploiter team is
        # optional when ``audit.replication.exploiter == 0`` — the deep
        # finding loop synthesizes a skipped placeholder for every
        # validator-surviving finding instead of dispatching the team.
        # analyzer / validator teams remain mandatory in deep mode.
        if self.teaming_enabled and (
            self.analyzer_team is None
            or self.validator_team is None
            or (
                self.exploiter_team is None
                and config.audit_mode.replication.exploiter != 0
            )
        ):
            raise ValueError(
        ...
        *,
        ...
        if config.audit_mode.mode == "deep":
            ...
            if agentic_form:
                ...
                # ``allow-zero-exploiter-replication``: skip persona
                # resolution when the exploiter stage is disabled —
                # ``resolve_personas`` rejects ``replication < 1`` and
                # the result is unused anyway (no team to feed).
                if config.audit_mode.replication.exploiter > 0:
            ...
            # ``allow-zero-exploiter-replication``: skip exploiter-team
            # construction entirely when the operator has opted the
            # stage out. The deep finding loop short-circuits on
            # ``self.exploiter_team is None`` and synthesizes the same
            # skipped placeholder used by the FP short-circuit.
            if config.audit_mode.replication.exploiter == 0:
                exploiter_team = None
            else:
    ...
    def _process_unit_single(
        ...
        *,
        ...
        while len(accepted_chain) < cap and consecutive_no_issue < 2:
            ...
            if self.config.audit_mode.replication.exploiter == 0:
                # ``allow-zero-exploiter-replication``: the operator
                # has opted out of the exploitation stage for the whole
                # run. Synthesize the same skipped placeholder shape the
                # FP short-circuit above uses so the downstream
                # ``not_exploitable`` downgrade path (status check
                # below) and the per-finding payload flow remain
                # uniform. The ``not_exploitable`` downgrade cannot
                # trigger here — its precondition is
                # ``status == "not_exploitable"``, which never holds
                # for a skipped finding.
                exploitation = ExploitationResult(
                    status="skipped",
                    steps="Exploiter disabled (audit.replication.exploiter=0).",
                )
                if self.logger is not None:
                    self.logger.debug_kv(
                        "Exploitation skipped (audit.replication.exploiter=0)",
                        path=unit.path.path_fingerprint,
                        round=round_index,
                    )
            else:
    ...
    def _process_unit_teaming(
        ...
        *,
        ...
        # ``self.exploiter_team`` may legitimately be ``None`` when the
        # operator set ``audit.replication.exploiter == 0`` — the
        # per-finding loop below short-circuits via
        # ``elif self.exploiter_team is None`` in that case.
        ...
        for finding_index, analyzer in enumerate(consolidated):
            ...
            elif self.exploiter_team is None:
                # ``allow-zero-exploiter-replication``: the operator
                # set ``audit.replication.exploiter == 0``. Synthesize
                # the same skipped placeholder shape used by the FP
                # short-circuit so the per-finding payload flow stays
                # uniform; the validator-team verdict is the final
                # per-finding verdict in this mode.
                if self.logger is not None:
                    self.logger.debug_kv(
                        "Exploiter team skipped",
                        path=unit.path.path_fingerprint,
                        finding=finding_fingerprint,
                        reason="audit.replication.exploiter=0",
                    )
                exploitation_result = ExploitationResult(
                    status="skipped",
                    steps="Exploiter disabled (audit.replication.exploiter=0).",
